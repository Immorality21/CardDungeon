...
- **Progression is the sphere grid, not levels.** `SphereGridSO` holds `Nodes` (`SphereGridNode`: stable string `Key` — save data, write-once — `Kind` Stat/Resistance/MagicSlot/**MagicKnown**, per-node `XpCost`, an optional `MaterialCosts`, an `UnlockedByDefault` flag, a `Gains` StatBlock or resistance/slot payload, authored 2D `Position` for the UI, and undirected `Neighbors` key lists) plus a `StartNodeKey`. **All rules live in `SphereGridOps`** (pure, static, scene-free): reachability is start-node ∥ adjacent-to-**active**, `TryActivate` is the validate→spend→append core, `SanitizeActivated` drops unknown keys but keeps unreachable-but-paid nodes, `GreedySpend` is the deterministic budget spend the balance model runs on (cheapest frontier first, ties by node index), and `StarterBank` (55% of the roster's average lifetime XP) seeds every new recruit — tavern (`HeroRoster.TryAddOwned`), rescue (`Party.AddHero`) and the balance model all call the same function. Covered by `SphereGridOpsTests`.
- **"Active" is not "saved": `SphereGridOps.ActiveNodes(grid, saved)` is what every rule reads.** It
  is the saved keys plus every `UnlockedByDefault` node, deduped. Going through one function is what
  makes a default unlock grant its payload, open its neighbours and refuse to be bought a second time
  without any of those three being remembered separately — and it is why marking an existing node
...
- **`UnlockedByDefault` is how a hero arrives holding something.** The **Warrior starts knowing
  ...
  **price it 0** (`SphereGridCostCurveTests` fails otherwise, and a non-zero price only misreports
  ...
  path — `HeroRoster.MarkDefaultUnlocksDiscovered`, called on hub load, or the Forge will refuse to
  upgrade the one spell every hero owns. Arming the other five heroes is one checkbox each.
- **A node's frontier grows only out of what the hero holds.** Until 2026-09-05 adjacency to the
  *start node* granted reachability whether or not the start had been bought — so a new hero's
  **second** node was purchasable while the first still read as unbought, and the entry node could be
  skipped outright. The start node is still always reachable (a fresh hero needs somewhere to begin);
  nothing else is until a neighbour is active.
- **`MaterialCosts` is a second price a player cannot grind past at the hub.** XP is banked from
  kills and materials only drop in a run, which is what makes materials the right lock for the far
  end of a branch — `warrior-b-cry` (War Cry) is the first one: 350 XP **and** 2 Ember Iron + 1 Void
  Shard. It lives outside `SphereGridOps.CanActivate` on purpose, because the balance model spends
  grids with no inventory to spend from; `HeroRoster.TryActivateNode` checks both prices before
...
- **Ownership is separate from the catalog.** `PartySaveData.OwnedHeroKeys` holds the save keys the player actually owns, read/written through **`HeroRoster`** (`GetOwnedHeroes`, `Owns`, `TryAddOwned`, `GetUnownedHeroes`). A new save starts with `PartyRosterSO.StartingHeroes` — the **Warrior alone**, and it must stay that way: the player starts with one hero and unlocks the rest (`HeroUnlockTests.Roster_StartsWithExactlyOneHero` fails otherwise, because a second starting hero is a balance change wearing an authoring change's clothes). Anything in the catalog but unowned is still to be unlocked. Legacy saves are migrated on first read: whoever already had a `HeroSaveData` entry counts as owned, so nobody loses a hero.

  **A hero in `StartingHeroes` cannot also be a captive.** `PlaceCaptiveIfConfigured` skips a hero the player already owns, so listing one in both places silently deletes the rescue — which is exactly what had happened to the tutorial's Paladin until 2026-09-06.
- **Heroes are unlocked, never bought** (`NEXT_STEPS.md` §5b). The only route today is **rescue**: `RunLevelEntry.RescueHero` places a captive in a non-start / non-exit room; freeing them (`DungeonManager.TryRescueCaptive`) adds them to the live party at once and records ownership **deferred**, so it commits on level clear and is forfeited on death exactly like XP. `HeroRoster.TryAddOwned` is the immediate hub-side writer, kept for whatever unlock source lands next. Two captives are authored — the Paladin (tutorial floor 1) and the Ranger (The Reedcage, Drowned March floor 2); the Cleric, Cultist, Tinkerer and Rogue have no source yet and are unreachable until they get one.
- **A hero is a key the campaign can gate on.** `CampaignNodeEntry.RequiresHeroes` + `CampaignOps.HeroGateSatisfied`: always all-of, ANDed with the run gate, and **failing shut** when a caller passes no owned set. The hard rule is that a gate must name a hero the player is *guaranteed* to hold on arrival — `CampaignOps.GetGuaranteedHeroKeys` walks only the **required** path, so a captive down an optional branch or behind an `Any` fork does not count, and `GetNodesWithBrokenHeroGates` (swept by `HeroUnlockTests`) fails the build if you gate on one. Static-analysis callers with no roster pass `CampaignOps.IgnoreHeroGate`.
...
- **Party width is four, free, from the first run** *(2026-09-17)*. `PartySlots` is now one constant (`MaxCap` 4). **The slot purchase was removed**, and the reason is that it was the last surviving piece of the tavern: §5b decided *gold never buys a hero again — a hero is access, gated by the campaign*, and buying the right to **field** one more hero is that same trade wearing a different hat. What paces width is the **roster** (rescues and campaign gates), and what prices it is the **XP split** — which was always the real cost, so nothing about the decision flattened when the toll came off. `MetaProgressSaveData.BonusPartySlots` is gone. The cap is still passed *into* `HeroRoster` rather than read there, so selection stays testable and free of the meta-progress singleton.
- **`PartySlots.FreeWidth` (1) is a different question from `MaxCap` (4).** The cap is what the game permits; `FreeWidth` is what a fresh save actually fields — the solo start — and it is what `InvestmentFrontier` charges width from. Pricing from the cap would hand the balance model three heroes nobody went and earned.
...
- **The campfire decides how the XP is divided** *(2026-09-17, `CampfireOps`)*. Three modes, and a campfire **building level** is what grants them: level 1 **Even**, level 2 **Mentor** (a hero the player names takes a double share), level 3 **Catch Up** (the double share goes to whoever has the least banked XP, re-chosen on every award). Mentor and Catch Up are the *same* mechanic differing only in who is picked, so there is one code path and one number to tune (`XpSplit.FavouredShares`, 2). **Every mode hands out the identical total** — favouring is redistribution, never a bonus — which is what keeps the balance model's XP accounting true whatever the player chose, and what makes this a legal thing for a building level to grant at all (access and control, never a raw stat). `Party.DistributeXp` resolves the favoured hero **per award**, because Catch Up's answer changes as the run pays out. Read the mode through `MetaProgressManager.GetXpSplitMode()`, never the raw save field: `CampfireOps.EffectiveMode` refuses a mode the fire does not offer, so a hand-edited save cannot run a mode it never unlocked.
- **Two XP rules that are choices, not consequences** (both in `XpSplit`): the integer-division **remainder is never dropped**, because silently losing up to `partySize - 1` XP per kill would make wide parties worse than the split implies — it goes to the leader on an even split, and to the **favoured hero** when one is named, so a mentor's share is unambiguously the largest at every award size rather than tying with the leader on a small one; and **downed heroes are paid**, unlike FFX, because excluding them punishes the tank role for doing its job. Death's cost stays HP and items.
