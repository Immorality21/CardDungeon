...
- **Audio lives in its own subsystem now**: `Assets/Scripts/Audio/` (namespace `Assets.Scripts.Audio`), moved out of `Combat/Audio` when the music bed arrived — the bed and the volume dials serve the hub as much as they serve a fight. `CombatAudio.Play(CombatSound.X)` is still the one-shot call from `CombatManager` (attacks/cast/draw/item/heal/boss wind-up/death/victory/defeat) and `RoomActionUI` (command-menu cursor + confirm); combat also drives the music bed — `LevelMusic.PlayCombat(hasBoss)` on the way in, `MusicPlayer.Stop` on victory/defeat, `LevelMusic.PlayExploration()` when the victory summary is dismissed. Full details (banks, tracks, the Master-on-the-listener rule, why a track with no clips fades to silence, and the fact that **no music files exist yet**) → `Assets/Scripts/Audio/CLAUDE.md`.
