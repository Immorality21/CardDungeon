...
- **Home is `cd-window--tall` (88%), not the shared 62% cap.** Nine stacked menu buttons plus the purse do not fit in 62%; the flex layout squeezed the last child to **zero height** and Gold/Essence rendered outside the frame. `.cd-currency-row` is `flex-shrink: 0` so the purse is never the row that gives way.
